...
app.get('/allDiagnoses', (req, res) => {
  ...
  let statement = `SELECT date, doctor, concerns, symptoms, diagnosis, prescription 
                   FROM Appointment A 
                   INNER JOIN (SELECT * 
  ...
  con.query(statement, [email], function (error, results, fields) {
      if (error) {
          console.error('SQL error:', error); // Log the error for better debugging
          return res.status(500).json({ error: 'Database query error' }); // Return a user-friendly error message
      } else {
          return res.json({
              data: results
          });
